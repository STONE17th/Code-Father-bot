...
adv_channel = 1001394024374087700
...
adv_timer = 10800
adv_title = ''
adv_text = f'Не дадим технологиям захватить мир! Машины должны батрачить на людей, а не наоборот. Вступай в ряды ботоводов ✊\nКурсы по телеграм-ботоводству на базе библиотеки AIOgram языка Python\nЗа подробностями в личку '
...
async def tyu0(ctx):
    embed = discord.Embed(title="Манифест сервера CODE Father`s",
                          description="Основная тематика сервера **CODE Fathers's** (далее CF) - IT-технологии, а именно направление '*Разработчик*' (кодинг, тестирование, аналитика, проджект и продукт менеджмент). Если тебе это знакомо - *welcome*.\nСервер создан студентами **GeekBrains** и изначально нацелен на обсуждение процесса обучения и сопутствующих тем, однако *тематика продолжает расширяться*. Свежие идеи по развитию контентной нагрузки *приветствуются*.\nНазвание '**CODE Father's**' результат игры слов от '*GodFather*' (Крестный отец) и никак не претендует на '*отцовство*' в сфере кодинга. На сервере присутствует достаточно много отсылок к тематике '*Крестоного отца*'.\nТак же на сервере присутствует достаточно много '*игровых*' моментов, что привносит интересные механики и разбавляет серые будни. Эти правила придется принять или нам с тобой не по пути.\n\nОбщие правила поведения.\n\n    **1. Основные принципы.**\n    **2. Оформление аккаунта.**\n    **3. Публикации.**\n    **4. Общение в голосовых каналах.**\n    **5. Роли на сервере. Получение первой роли.**\n    **6. Семья.**\n    **7. Бот CODE Father.**\n    **8. Кары, ответственность.**",
                          color=0x8400ff)
...
async def tyu1(ctx):
    embed = discord.Embed(title="1. Основные принципы",
                          description="    **1.1 Будьте вежливы**. За грубостями и оскорблениями последует кара вплоть до прощания с сервером.\n    **1.2 Мат не воспрещается**, но старайтесь держаться в рамках, не надо '*разговаривать матом*'.\n    **1.3** Старайтесь использовать **общепринятую техническую терминологию** (или общепринятый сленг), ясно и четко излагать суть проблемы.\n    **1.4** При ответе на какой-либо вопрос или высказывание, именно отвечайте, кликнув '*ответить*' на исходном сообщении.\n    **1.5** При ответе на вопрос старайтесь придерживаться тех же правил - техническая терминология, ясность, четкость, суть. Не стоит сразу отправлять вопрошающего в *гугл* (там он, видимо, уже был), а вот ссылка на ресурс с ответом/документацией приветствуется, не стесняйтесь уточнить куда смотреть в первую очередь.\n    **1.6** Задавая вопрос по ошибке интерпретатора приводите **скриншот** (или *листинг*) ошибки и, желательно, соответствующий фрагмент кода (скриншот/листинг).",
                          color=0x00ff7b)
    await ctx.send(embed=embed)
...
async def tyu2(ctx):
    embed = discord.Embed(title="2. Оформление аккаунта",
                          description="    **2.1.1 Ник-нейм**. Крайне приветствуются реальные имена или же псевдонимы исключающие *двусмысленность*. Недопустимы **шовинистические** или **радикальные** псевдонимы. По-возможности избегайте привязывать ник к *политике* или *религии*.\n    **2.1.2 Служебные символы** - @, #, $, %, ^, &, *, ?, |, /, , а также *эмодзи* и подобное недопустимы в следствие технических ограничений базы данных, вы будете вне базы и покинете сервер.\n    **2.2 Аватар**. Мы всегда будем рады видеть Вашу улыбку в наших рядах, если же Вы желаете ее скрыть - это *Ваш выбор*. **Недопустимы**: порнография, мертвечина и прочее так-называемое гуро.\n    **2.3 Пользовательский статус**. По вкусу. Недопустимы *оскорбления*, *радикальные*, *экстремистские высказывания*.",
                          color=0xb00e6a)
    await ctx.send(embed=embed)
...
async def tyu3(ctx):
    embed = discord.Embed(title="3. Публикации",
                          description="    **3.1** Публикуемые материал должны соответствовать *основной тематике сервера* - IT.\n    **3.2** Для отвлеченных рассуждений предусмотрена *флудильня*.\n    **3.3** При публикации сторонних материалов, соблюдайте *копирайт авторов*. Проверяйте, разрешил ли атор публикацию на других ресурсах, нежели исходный, обязательно оставляйте **ссылку на оригинал**. В случае бумажного исходника - *автор, название, год издания, страница*.\n    **3.4** **Запрещается** публиковать материалы, представляющие собой *коммерческую тайну*!\n    **3.5** Публикация материалов, являющихся чьей-либо *интеллектуальной собственностью* допустима с **только согласия атора** (*владельца*) с явным указанием данного согласия.\n    **3.6** Публикация материалов находящихся в открытом (*бесплатном*) доступе (документы pdf, doc, txt, электронные/сканированные книги, статьи) допускается только в виде **ссылки на исходный ресурс**.\n    **3.7** **Запрещается** публикация *экстремистских* и *радикальных* материлов! За нарушения будет вынесен **БАН**.\n    **3.8** Публикация *политических/религиозных* материалов, а также *обсуждения* на данные темы **ЗАПРЕЩАЮТСЯ** в с связи со складывающейся ситацией в стране. *Данная тематика не соответствует тематике данного сервера*. За нарушения будет вынесен **БАН**.",
                          color=0xfff200)
    await ctx.send(embed=embed)
...
async def tyu4(ctx):
    embed = discord.Embed(title="4. Общение в голосовых каналах. Подкасты",
                          description="    **4.1** На мероприятиях присутствуют *докладчик* и *ведущий*. Вопросы докладчику задаются *письменно* через ведущего. В конце мероприятия допустима *открытая голосовая сессия* вопросов к докладчику.\n    **4.2** **Недопустимы оскорбления** в адрес *докладчиков* и *участников* на голосовых мероприятиях.\n    **4.3** Общение в голосовых каналах свободное в отсутствие плановых мероприятий в рамках обозначенной *тематики канала*. **Будьте взаимно вежливы**.",
                          color=0x00ffee)
    await ctx.send(embed=embed)
...
async def tyu5(ctx):
    embed = discord.Embed(title="5. Роли сервера",
                          description="    **5.1** На сервере предусмотрена иерархия *ролей* с различным *уровнем доступа* текстовым и голосовым каналам сервера **CF**.\n    **5.2** Каждый новый участник сервера *не имеет никакой роли* и имеет минимальный доступ (*общеинформационные каналы, флудильня и общий голосовой канал*). Участник в **течение трех дней** (с момента вступления на сервер) не получивший роли - **удаляется с сервера**.\n    **5.3 Серая роль - 'NPC'**. Получается *автоматически* после принятия данного манифеста. Для принятия - в любом голосовом чате (в том числе и в личных сообщениях с ботом CF) надо отправить команду **/access**\n    **5.4 Зеленая роль - 'NewCoder'**. Дает доступ к *полезным голосовым и текстовым каналам* сервера CF. Выдается ботом CF.\n    **5.5 Синяя роль - 'CapoCoder'**. Обладает *расширенным функционалом* и выдается участникам которые хотят создать '*семью*' (подробнее о 'семьях' можно прочитать в 6 пункте). Заявку на создание 'семьи' можно подать через *бота*.\n    **5.6 Фиолетовая роль - 'UnderCode'**. Выдается *крайне полезным* людям, которые активно участвуют в жизни сервера и помогают остальным жителям. Выдается админами или при достижении *CF-рейтинга 100*.\n    **5.7 Оранжевая роль - 'CODE Father'**. Макисмальная (почти :)) по функционалу и значимости роль на сервере CF. К обладателям этой роли *всегда можно обратиться за помощью* или с вопросом/предложением относительно сервера. Выдается **исключительно админами**, является повышением от роли '*UnderCode*'",
                          color=0xff8c00)
    await ctx.send(embed=embed)
...
async def tyu6(ctx):
    embed = discord.Embed(title="6. Семья",
                          description="    **6.1** '**Семья**' - команда *единомышленников* трудящихся над одним проектом.\n    **6.2** Семье выдается *именной скрытый голосовые чат* (чтоб никто не мешал), доступ к эти каналам имеют только члены семьи.\n    **6.3** В семью может принять (выдать роль) *глава семьи* путем команды **CF боту**.\n    **6.4** Объединение в семьи **крайне приветствуется** на просторах **сервера CF**.",
                          color=0x0400ff)
    await ctx.send(embed=embed)
...
async def tyu8(ctx):
    embed = discord.Embed(title="7. Бот 'CODE Father'",
                          description="    **7.1** На сервере **24/7** несет свою службу **Code Father бот**, написанный админами сервера для автоматизации процессов.\n    **7.2** *Предложения по расширению функционала бота* - направлйте в личку **админам**. Это очень приветствуется.\n    **7.3** Более подробную информацию можно получить по команде **/info**",
                          color=0xff00fb)
    await ctx.send(embed=embed)
...
async def tyu9(ctx):
    embed = discord.Embed(title="8. Кары, ответственность",
                          description="    **8.1** На сервере предусмотренно *2 вида* пресечения свободы действий:\n    **8.2.1 Бан на общение**. Длительность бана зависит от тяжести '*преступления*'.\n    **8.2.2 Удаление с сервера** и занесение *ID в черный список*.\n    **8.3** После получения **8.2.1** рейтинг участника *обнуляется* и роль сбрасывается до *первого уровня*.\n    **8.4** После *трехкратного* получения **8.2.1** участник получает **8.2.2**",
                          color=0xff0000)
    await ctx.send(embed=embed)
